defmodule RedixClustered.Conn do
  ...
  def command(name, cmd, max_redirects) do
    ...
    |> follow_redirects(&Redix.command/2, cmd, max_redirects)
  end
  ...
  def pipeline(name, cmds, max_redirects) do
    ...
    |> Registry.lookup(name)
    |> follow_redirects(&Redix.pipeline/2, cmds, max_redirects)
  end
  ...
  defp follow_redirects(pid, redix_fn, args, max) do
    follow_redirects(pid, redix_fn, args, max, 0)
  end

  defp follow_redirects(_pid, _redix_fn, _args, max, attempt) when attempt > max do
    {:error, "Max redis redirects reached after #{attempt}"}
  end

  defp follow_redirects(pid, redix_fn, args, max, attempt) do
    # TODO: ASK support
    case redix_fn.(pid, args) do
      {:error, %Redix.Error{message: "MOVED " <> moved}} ->
        moved
        |> Registry.connect()
        |> follow_redirects(redix_fn, args, max, attempt + 1)
